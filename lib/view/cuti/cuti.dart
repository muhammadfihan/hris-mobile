...
import 'package:hris_apps/view/cuti/updatecuti.dart';
...
class _CutiState extends State<Cuti> {
  ...
  Future detailcuti() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    var token =
        await localStorage.getString("token").toString().replaceAll('"', '');

    final response =
        await http.get(Uri.parse(baseURL + 'detailcutimobile/${id}'), headers: {
      'Content-Type': 'application/json; Charset=UTF-8',
      'Accept': 'application/json',
      'Authorization': 'Bearer $token',
    });
    var detcuti = json.decode(response.body);
    if (detcuti['message'] == "Unauthenticated.") {
      localStorage.remove('token');
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
  ...
  deletecuti() async {
    return QuickAlert.show(
        ...
        title: 'Hapus Izin',
        text: 'Apakah anda ingin menghapus cuti ?',
        ...
        onConfirmBtnTap: () async {
          ...
          final response = await http
              .delete(Uri.parse(baseURL + 'hapuscuti/${id}'), headers: {
            'Content-Type': 'application/json; Charset=UTF-8',
            'Accept': 'application/json',
            'Authorization': 'Bearer $token',
          });
          var delcuti = json.decode(response.body);
          if (delcuti['message'] == "Unauthenticated.") {
            localStorage.remove('token');
            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(
            ...
            return QuickAlert.show(
              ...
              title: 'Berhasil',
              text: 'Anda Telah Menghapus Cuti',
            );
          }
        });
  }
  ...
  updatecuti() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse(baseURL + 'updatecutimobile/${id}'));
                                  ...
                                  Container(
                                    ...
                                    child: Padding(
                                      padding: EdgeInsets.all(
                                          8), //apply padding to all four sides
                                      child: Text(
                                          snapshot.data['data']['status_cuti']),
                                    ),
                                  ),
                                  SizedBox(
                                    height: 20,
          ...
          IconButton(
            ...
            onPressed: () async {
              QuickAlert.show(
                ...
                text:
                    'Anda harus menunggu pengajuan cuti disetujui/ditolak untuk mengajukan pengajuan cuti kembali',
              );
            },
          ), //Icon
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (BuildContext context) => AjukanCuti(),
                  ));
            },
          ), //IconButton
        ], //<Widget>[]
                                ...
                                child: ClipRRect(
                                  ...
                                  child: Container(
                                    ...
                                    child: Column(
                                        ...
                                        children: [
                                          Text(
                                            "${snapshot.data['data'][index]['jenis_cuti']}",
                                            style: TextStyle(
                                                fontSize: 15,
                                          ...
                                          Row(
                                            children: [
                                              ...
                                              if (snapshot.data['data'][index]
                                                  ...
                                                  "Diterima") ...[
                                                Text(
                                                  "${snapshot.data['data'][index]['status_cuti']}",
                                                  style: TextStyle(
                                                    fontSize: 14,
                                          ...
                                          Row(
                                            children: [
                                              ...
                                              Text(
                                                "${snapshot.data['data'][index]['tanggal_cuti']}",
                                                overflow: TextOverflow.ellipsis,
                                                style: TextStyle(
                                                    fontSize: 14,
                                                    color: Colors.grey,
